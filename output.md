 +# Research Workflow Assistant Protocol v2
    ...
    +---
    +
    +### 2.3 Evidence Record（新增：证据门控记录，用于幻觉治理）
    ...
    +---
    +
    +### 2.4 Artifact Manifest（新增：产物清单，用于可追溯与审计）
    ...
    +- `role`
    +- `input_summary`（脱敏/截断）
    +- `output_summary`（脱敏/截断）
    +- `artifact_refs`（相对路径数组）
    +
    +### 8.3 TraceEvent（JSONL）示例
    +```json
    +{"ts":"2026-01-19T00:00:00Z","event":"model_call.started","run_id":"run-20260119-xxxx","task_id":"task-20260119-xxxx","backend":"codex","role":"reasoner","session_id_in":"<runtime>","input_summary":"...","artifact_refs":[]}
    +{"ts":"2026-01-19T00:00:10Z","event":"model_call.completed","run_id":"run-20260119-xxxx","task_id":"task-20260119-xxxx","backend":"codex","role":"reasoner","session_id_out":"<runtime>","output_summary":"...","artifact_refs":[".research/tasks/<task_id>/model_out/codex.reason     
    er.json"],"ok":true,"duration_ms":10000}
    +```
    +
    ...
    +- `role_file` 必须在 `.claude/.research/prompts/` 下（相对路径），不得引用 `.ccg` 或用户目录绝对路径
    +
    ...
    ---
    SESSION_ID: 019bd705-7265-7f00-abff-93e2ed964478